...
export default function ChapterContent({
      ...
      try {
        ...
        if (chapterRes.ok) {
          ...
          const publishedChapters = chaptersData.chapters
            ...
            .sort(
              (a: Chapter, b: Chapter) => a.publish_order - b.publish_order
            );
  ...
  useEffect(() => {
    ...
    if (isCompleted && !isMarkedAsRead) {
      markChapterAsRead();
    }
  }, [chapter?.dialogues, visibleDialogues.length, isMarkedAsRead]);
  ...
  const isChapterCompleted =
    chapter?.dialogues && visibleDialogues.length === chapter.dialogues.length;
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-indigo-900 relative overflow-hidden">
        ...
        <div className="flex relative z-10">
          ...
          <div className="hidden lg:block w-72 h-screen">
            <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl shadow-xl border-r border-white/20 dark:border-slate-700/50 h-full">
              <div className="p-4">
                <Skeleton
                  height={32}
                  width="80%"
                ...
                <div className="space-y-2">
                  {Array(10)
                    .fill(0)
                    .map((_, i) => (
                      <Skeleton key={i} height={36} className="rounded-2xl" />
                    ))}
                </div>
              </div>
            </div>
          ...
          <div className="flex-1">
            <div className="container max-w-2xl mx-auto px-4 py-8">
              ...
              <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-3xl shadow-xl border border-white/20 dark:border-slate-700/50 min-h-[50vh] max-h-[70vh] p-6">
                          ...
                          >
                            <div
                              className={`bg-gradient-to-r ${
                                index % 2 === 0
                                  ? "from-blue-50/50 to-purple-50/50 dark:from-blue-900/10 dark:to-purple-900/10"
                                  : "from-green-50/50 to-blue-50/50 dark:from-green-900/10 dark:to-blue-900/10"
                              } rounded-2xl p-3 border border-white/20 dark:border-slate-600/20`}
                            >
                              <Skeleton
                                width={200}
                                height={60}
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-indigo-900 relative overflow-hidden">
      ...
      <div className="flex relative z-10">
        ...
        <div className="hidden lg:block w-72 h-screen sticky top-0 overflow-y-auto">
          <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl shadow-xl border-r border-white/20 dark:border-slate-700/50 h-full">
            <div className="p-4">
              <h2 className="font-bold text-xl mb-4 bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 bg-clip-text text-transparent">
                {storyTitle}
              </h2>
              <div className="space-y-2">
                {allChapters.map((c) => (
                  <motion.button
                    ...
                    onClick={() => {
                      startLoading(
                        `/library/${storyId}/chapters/${c.chapter_id}`
                      );
                      router.push(
                        `/library/${storyId}/chapters/${c.chapter_id}`
                      );
                    ...
                    className={clsx(
                      "w-full text-left px-3 py-2 rounded-2xl transition-all duration-300",
                      Number(chapterId) === c.chapter_id
                        ? "bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg"
                        : "bg-white/50 dark:bg-slate-700/50 hover:bg-white/80 dark:hover:bg-slate-700/80 border border-white/20 dark:border-slate-600/20"
                    )}
                  >
                    {c.title}
                  </motion.button>
                ...
                {visibleDialogues.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-muted-foreground py-20">
                    ...
                    <p className="mt-4 text-lg">
                      Nhấn vào màn hình để bắt đầu đọc
                    </p>
                  </div>
                ) : (
                  <AnimatePresence>
                    {chapter.dialogues.map(
                      (dialogue) =>
            ...
            <div className="fixed bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-background/80 backdrop-blur-sm p-2 rounded-full border shadow-lg">
              {prevChapter && (
                <Button
                  ...
                  onClick={() => {
                    startLoading(
                      `/library/${storyId}/chapters/${prevChapter.chapter_id}`
                    );
                    router.push(
                      `/library/${storyId}/chapters/${prevChapter.chapter_id}`
                    );
              ...
              {nextChapter && (
                <Button
                  ...
                  onClick={() => {
                    startLoading(
                      `/library/${storyId}/chapters/${nextChapter.chapter_id}`
                    );
                    router.push(
                      `/library/${storyId}/chapters/${nextChapter.chapter_id}`
                    );
