...
import {
  SYSTEM_PROMPTS,
  createStoryPrompt,
  createEditStoryPrompt,
  createCharacterPrompt,
  createEditCharacterPrompt,
  createCoverImagePrompt,
  createAvatarPrompt,
  createDialoguePrompt,
  createChapterPrompt,
  createEditChapterPrompt,
  createOutlinePrompt,
  createEditOutlinePrompt,
  createSearchPrompt,
} from "./gemini-prompts";
...
export async function generateStoryIdea(
  ...
  try {
    ...
    const chat = model.startChat({
      history: [createStoryPrompt(categories, tags)],
    });
    ...
    try {
      ...
      if (
        ...
        !storyIdea.mainCategory ||
        !Array.isArray(storyIdea.suggestedTags)
      ) {
...
export async function generateDialogueSuggestion(
  ...
  try {
    ...
    const result = await chat.sendMessage(
      prompt || "Hãy tạo một số đoạn đối thoại phù hợp"
    );
    const response = result.response.text();
    ...
    try {
      const parsedResponse = JSON.parse(
        jsonString
      ) as MultipleDialoguesResponse;

      if (
        !parsedResponse.dialogues ||
        !Array.isArray(parsedResponse.dialogues)
      ) {
...
export async function generateStoryEdit(
  ...
  try {
    ...
    const chat = model.startChat({
      history: [createEditStoryPrompt(categories, tags, existingStory)],
    });
    ...
    try {
      ...
      if (
        ...
        !editedStory.mainCategory ||
        !Array.isArray(editedStory.suggestedTags)
      ) {
...
export async function generateCoverImagePrompt(storyInfo: {
  title: string;
  description: string;
  mainCategory: string;
  tags: string[];
  ...
  try {
    ...
    const chat = model.startChat({
      history: [createCoverImagePrompt(storyInfo)],
    });
    ...
    throw new Error(
      "Có lỗi xảy ra khi tạo prompt ảnh bìa. Vui lòng thử lại sau."
    );
  }
}

export async function generateAvatarPrompt(characterInfo: {
  name: string;
  description: string;
  gender: string;
  personality: string;
  ...
  try {
    ...
    const chat = model.startChat({
      history: [createAvatarPrompt(characterInfo)],
    });
    ...
    throw new Error(
      "Có lỗi xảy ra khi tạo prompt avatar. Vui lòng thử lại sau."
    );
  }
}
...
export async function generateChapterIdea(
  ...
  try {
    ...
    const chat = model.startChat({
      history: [createChapterPrompt(storyContext, publishedChapters)],
    });
...
export async function generateOutlineIdea(
  ...
  try {
    ...
    const chat = model.startChat({
      history: [createOutlinePrompt(storyContext, publishedChapters)],
    });
...
export async function searchStoriesWithAI(
  ...
  try {
    ...
    const chat = model.startChat({
      history: [createSearchPrompt(stories)],
    });
