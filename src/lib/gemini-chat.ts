...
- ID: ${chapter.chapter_id}
  ...
  ${
    selectedStory.dialogues
      ?.find((d) => d.chapter_id === chapter.chapter_id)
      ?.dialogues.map(
        (dialogue) => `
    ...
    return new ReadableStream({
      async start(controller) {
        ...
        for await (const chunk of result.stream) {
          ...
          if (
            accumulatedText.includes("/create-story") ||
            ...
            const createDialogueMatch = accumulatedText.match(
            ...
            const storyMatch = accumulatedText.match(
              /\/create-story\s*({[\s\S]*?})/
            );
            const characterMatch = accumulatedText.match(
              /\/create-character\s*({[\s\S]*?})/
            );
            const chapterMatch = accumulatedText.match(
              /\/create-chapter\s*({[\s\S]*?})/
            );
            const outlineMatch = accumulatedText.match(
              /\/create-outline\s*({[\s\S]*?})/
            );
            const editMatch = accumulatedText.match(
              /\/edit-story\s*({[\s\S]*?})/
            );
            const editCharacterMatch = accumulatedText.match(
              /\/edit-character\s*({[\s\S]*?})/
            );
            const editChapterMatch = accumulatedText.match(
              /\/edit-chapter\s*({[\s\S]*?})/
            );
            const editOutlineMatch = accumulatedText.match(
              /\/edit-outline\s*({[\s\S]*?})/
            );
            const deleteCharacterMatch = accumulatedText.match(
              /\/delete-character\s*({[\s\S]*?})/
            );
            const deleteChapterMatch = accumulatedText.match(
              /\/delete-chapter\s*({[\s\S]*?})/
            );
            const deleteOutlineMatch = accumulatedText.match(
              /\/delete-outline\s*({[\s\S]*?})/
            );
            const deleteDialogueMatch = accumulatedText.match(
              /\/delete-dialogue\s*({[\s\S]*?})/
            );
            const editDialogueMatch = accumulatedText.match(
              /\/edit-dialogue\s*({[\s\S]*?})/
            );
            const publishStoryMatch = accumulatedText.match(
              /\/publish-story\s*({[\s\S]*?})/
            );
            const deleteStoryMatch = accumulatedText.match(
              /\/delete-story\s*({[\s\S]*?})/
            );
